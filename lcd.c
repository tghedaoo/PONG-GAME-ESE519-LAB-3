...
else if (dx == 0) // vertical
{
	...
	{
		setpixel(buff,x0,j,0x00);
	}
}

else if (dy/dx > 0) // Slope positive	
{
if (dy/dx <= 1)
{
...
for(int8_t i=x0-r;i<=x0+r;i++)
{
	for(int8_t j=y0-r;j<=y0+r;j++)
	{
		int8_t m = i-x0;
		int8_t n = j-y0;
		int8_t sm,sn,sr;
